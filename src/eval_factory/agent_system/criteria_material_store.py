...
class CriteriaRubricMaterialError(RuntimeError):
    pass


class CriteriaRubricMaterialStore:
    def __init__(
        self,
        root: Path,
    ) -> None:
        ...
        self.database = self.root / "criteria-materials.sqlite3"
        self._initialize()
    ...
    def put_model(
        ...
        *,
        ...
        content_ref = self.private_store.put_bytes(
            object_type="criteria-private-material",
            payload=payload,
        )
        ...
        try:
            ...
            existing = connection.execute(
                """
                SELECT * FROM criteria_materials
            ...
            if existing is not None:
                if (
                    ...
                    raise CriteriaRubricMaterialError(
                        "criteria behavior ref is already bound to different material"
                    )
                connection.rollback()
                return content_ref
            connection.execute(
                """
                INSERT INTO criteria_materials (
    ...
    def get_model[ModelT: ContractModel](
        ...
        try:
            ...
            if row is None:
                raise CriteriaRubricMaterialError("criteria private material is missing")
            if (
                ...
                raise CriteriaRubricMaterialError("criteria behavior ref binding drifted")
        ...
        try:
            ...
            raise CriteriaRubricMaterialError("criteria private material has the wrong schema") from exc
        if value.canonical_json() != payload:
            raise CriteriaRubricMaterialError("criteria private material is not canonical")
        return value
    ...
    def _initialize(self) -> None:
        ...
        try:
            ...
            connection.execute(
                """
                CREATE TABLE IF NOT EXISTS criteria_materials (
